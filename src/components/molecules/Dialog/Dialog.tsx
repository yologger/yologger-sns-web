...
import { IDialog, IButton } from '~/types';
              ...
              {dialogs &&
                dialogs.map((dialog) => (
                  <React.Fragment key={dialog.label}>
                    {dialog.actionType === 'button' && <DialogButton onClick={dialog.callback}>{dialog.label}</DialogButton>}
                    {dialog.actionType === 'anchor' && <DialogAnchor href={dialog.url}>{dialog.label}</DialogAnchor>}
                    {dialog.actionType === 'link' && <DialogLink to={dialog.path}>{dialog.label}</DialogLink>}
                  </React.Fragment>
                ))}
              {buttons && (
                <Buttons direction={buttonDirection || 'row'}>
                  {buttons.map((button) => {
                    switch (button.actionType) {
                      case 'anchor':
                        return (
                          <ButtonAnchor href={button.url} grow={button.grow || 1} buttonStyle={button.style} key={button.label}>
                            {button.label}
                          </ButtonAnchor>
                        );
                      case 'link':
                        return (
                          <ButtonLink to={button.path} grow={button.grow || 1} buttonStyle={button.style} key={button.label}>
                            {button.label}
                          </ButtonLink>
                        );
                      default:
                        return (
                          <Button onClick={button.callback} grow={button.grow || 1} buttonStyle={button.style} key={button.label}>
                            {button.label}
                          </Button>
                        );
                    }
